...
if [ "$startingCore" -eq 0 ]; then
    ...
    cmd="$DIR_PATH/node-$version-$os-$architecture --signature-check=false"
    ...
    for core_num in $(seq 1 $((maxCores - 1))); do
        cmd="sudo chrt -f 77 $DIR_PATH/node-$version-$os-$architecture --core=$core_num --parent-process=$parent_pid -signature-check=false"
    ...
    for core_num in $(seq $((startingCore + 1)) $((startingCore + maxCores))); do
        cmd="sudo chrt -f 66 $DIR_PATH/node-$version-$os-$architecture --core=$core_num -signature-check=false"
        echo "DEBUG: Deploying core $core_num with command: $cmd"
        $cmd &
        worker_pids[$core_num]=$!
    done
